...
def run_profiling(request):
    ...
    if serializer.is_valid():
        ...
        entry_point_path = serializer.resolve_entry_point_path()
        ...
        if serializer.validated_data['mode'] == "time" or serializer.validated_data['mode'] == "memory":
            deepview_output_file = "profile_output.sqlite"

            exit_code, output, error_output = profile_time_and_memory(serializer, work_dir, deepview_output_file)
        ...
        elif serializer.validated_data['mode'] == "analysis":
            deepview_output_file = "profile_output.json"

            exit_code, output, error_output = profile_analysis(serializer, work_dir, deepview_output_file)
            ...
            try:
                with open(deepview_output_file_path, "r") as profile_json:
                    json_data = json.load(profile_json)

                    return Response(json_data)
            except:
                log.error("Error loading json output: %s", deepview_output_file_path)
                return Response(
                    {
                        "error": "Could not load json output file",
                        "exit_code": exit_code,
                        "output": output,
                        "errors": f"Error loading json output file: {deepview_output_file_path}"
                    }
                )
    else:
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

# Running deepview_profile time and memory    
def profile_time_and_memory(serializer, work_dir, deepview_output_file):
...
# Running deepview_profile in analysis mode
def profile_analysis(serializer, work_dir, deepview_output_file):
    # Running DeepView.Profile module 
    selected_analysis_type = "--all"
    ...
    run_params.append("-o")
    run_params.append(deepview_output_file)

    log.debug("Working directory: %s", work_dir)
    log.info("Running DeepView.Profile with param: %s", run_params)

    process_result = subprocess.run(run_params, 
